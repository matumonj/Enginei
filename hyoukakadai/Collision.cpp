...
bool Collision::GoalCollision(int map[20][200], std::unique_ptr<Object3d> tst[20][200], float mapx[20][200], float mapy[20][200], const int X, const int Y, float& grav, float& time, float& movespeed, bool& jumpf, XMFLOAT3& Player_Pos, XMFLOAT3 Player_Scl, XMFLOAT3& Old_Pos, int MAP_NUMBER)
{
	float height;//
	float width;
	for (int i = 0; i < X; i++) {
...
void Collision::BossMap(int map[50][50], std::unique_ptr<Object3d> tst[50][50], float mapx[50][50], float mapy[50][50], const int X, const int Y, float& grav, float& time, float& movespeed, bool& jumpf, XMFLOAT3& Player_Pos, XMFLOAT3 Player_Scl, XMFLOAT3& Old_Pos, int MAP_NUMBER)
{
	float height;//
	float width;
	for (int i = 0; i < X; i++) {
